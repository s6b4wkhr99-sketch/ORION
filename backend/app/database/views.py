...
VIEW_DDL: dict[str, str] = {
    ...
    """,
    "vw_campaign_summary": """
        CREATE VIEW IF NOT EXISTS vw_campaign_summary AS
        SELECT
            camp.campaign_id,
            camp.campaign_name,
            camp.campaign_type,
    ...
    """,
    "vw_state_summary": """
        CREATE VIEW IF NOT EXISTS vw_state_summary AS
        SELECT
            c.state,
    ...
    """,
    "vw_zip_summary": """
        CREATE VIEW IF NOT EXISTS vw_zip_summary AS
        SELECT
            c.zip AS zip_code,
            c.state,
            COUNT(c.customer_id) AS customer_count,
    ...
    """,
    "vw_roi_summary": """
        CREATE VIEW IF NOT EXISTS vw_roi_summary AS
        SELECT
            cs.campaign_id,
...
INDEX_DDL: list[str] = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_customer_upload ON customers (upload_id)",
    # Covering index so executive-dashboard state/global aggregates run as
    # index-only scans instead of scanning the 5GB+ customer_intelligence heap.
    (
        "CREATE INDEX IF NOT EXISTS idx_ci_exec_covering ON customer_intelligence (customer_id) "
        "INCLUDE (purchase_power_index, pain_index, lifestyle_index, email_response_index, "
        "brand_familiarity_index, campaign_priority, prizm_proxy_segment, ceragem_segment, recommended_product)"
    ),
